...
const socket = io();
let state = { settings:{ theme:'dark', platform:'chaturbate', chaturbate:{} }, mappings:[], patterns:[], overlay:{ elements:[] } };
...
socket.on('state:init', s=>{
  ...
  const cb = state.settings?.chaturbate||{}; cbType.value = cb.feedType||'longpoll'; cbUrl.value=cb.url||''; cbToken.value=cb.token||''; cbRoom.value=cb.room||'';
...
// Save CB + SaveAll
saveCB.onclick = ()=>{ const cfg={ feedType: cbType.value, url: cbUrl.value, token: cbToken.value, room: cbRoom.value }; socket.emit('platform:set','chaturbate'); socket.emit('platform:update', cfg); toast('Chaturbate settings saved'); };
