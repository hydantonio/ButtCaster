...
import { connectIntiface, vibrateAll } from './intiface.esm.js';
...
const state = {
  overlay: { goalTarget: 2000, goalProgress: 0, glow: true, watermark: true, elements: [] },
  devices: [],
  intiface: { url: 'ws://127.0.0.1:12345', connected: false },
  bpClient: null
};
...
app.post('/api/intiface/connect', async (req,res)=>{
  try{
    const url = req.body?.url || state.intiface.url;
    state.intiface.url = url;
    await connectIntiface(url, state, io);
    res.json({ ok:true, devices: state.devices });
  }catch(e){ res.status(500).json({ ok:false, error: String(e) }); }
});
app.get('/api/devices', (req,res)=> res.json({ ok:true, devices: state.devices, connected: state.intiface.connected }));
app.post('/api/emit-tip', async (req,res)=>{
